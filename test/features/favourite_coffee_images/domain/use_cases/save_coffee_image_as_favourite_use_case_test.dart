...
void main() {
  ...
  late final SaveCoffeeImageAsFavouriteUseCase useCase;

  setUpAll(() {
    useCase = SaveCoffeeImageAsFavouriteUseCase(repositoryMock);
  });
  ...
  test('Should return success when image is already favourite', () async {
    // Arrange
    when(() => repositoryMock.getFavouriteCoffeeImages())
  ...
  test('Should properly call repository to save a new list of images',
    ...
    expect(response, const Right(null));
    verify(() => repositoryMock.getFavouriteCoffeeImages()).called(1);
    final newList = [tCoffeeImage, ...tCoffeeImages];
